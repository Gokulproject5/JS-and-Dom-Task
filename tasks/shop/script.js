...
function showAlert(message, color) {
 ...
 my-2 text-white text-center item-center  text-sm font-sans font-medium animate-[slideIn_0.5s_ease] transition-all duration-600 delay-200
  `;
...
const createFunction = (event) => {
  ...
  if (!value)
    return showAlert(
      `<i class="fa-solid fa-circle-info"></i> Enter the Bucket List`,
      "yellow",
    );
  ...
  // LI styles
  liEl.className =
    "bg-gray-300 shadow text-sm rounded text-gray-900   px-4 py-2 flex justify-between hover:bg-gray-200 ";
  liEl.innerText = "";
  // Text
  ...
  editBtn.className =
    "bg-green-400 text-white px-2 py-1 rounded hover:bg-green-500";

  editBtn.addEventListener("click", () => {
    if (editBtn.innerHTML === editBtnCheck) {
      ...
      editBtn.innerHTML = editBtnCheck;

      showAlert(
        `<i class="fa-solid fa-circle-check"></i> Successfully updated: ${newValue}`,
        "yellow",
      );
    }
  });
  ...
  deleteBtn.addEventListener("click", () => {
    ...
    if ((ulEl.children.length = 1)) {
      liEl.remove();
      if (ulEl.children.length < 1) ulEl.classList.remove("bg-white/10", "shadow");
    } else {
      liEl.remove();
      cartCountEl.classList.remove("bg-green-500");
      cartCountEl.classList.add("bg-red-500");
    ...
    showAlert(
      `<i class="fa-solid fa-circle-minus"></i> Deleted: ${textSpan.textContent}`,
      "red",
    );
  });
  ...
  ulEl.className = "bg-white/10 backdrop-blur-lg rounded-md px-5 py-5 shadow space-y-2";
  ulEl.append(Fragment);
  showAlert(
    `<i class="fa-solid fa-circle-check"></i>  Added: ${value}`,
    "green",
  );
  inputEl.value = "";
};
...
clearAll.addEventListener("click", () => {
  if (ulEl.children.length > 0) {
    ulEl.classList.remove("bg-white/10", "shadow");
